...
from celery_app import start_instance_task, set_notifications_task, end_polling_task, change_keys_task, new_sheet_task
import jsonpickle
...
# @app.task
# def start_instance_task(message):
...
def start_instance(message):
    start_instance_task.delay(message.id, message.text, message.chat.id)
...
# @app.task
# def set_notifications_task(message):
#     with Session(bind=engine, expire_on_commit=True) as session:
#         with session.begin():
#             try:
...
def set_notifications(message):
    set_notifications_task.delay(message.id, message.text, message.chat.id)
...
def end_polling(message):
    end_polling_task.delay(message.id, message.text, message.chat.id)
...
def change_keys(message):
    change_keys_task.delay(message.id, message.text, message.chat.id)
...
def new_sheet(message):
    new_sheet_task.delay(message.id, message.text, message.chat.id)
